...
IMPLICIT NONE
    ...
    REAL(8) :: cvar(ncomm,ncomm)
        ...
        sigp = 0.0d0
        i = i + 1
        sigp (1,1) = 1.0d0 * DEXP(theta(i))/(1+DEXP(theta(i))) 
        i = i + 1
        sigp (2,2) = 1.0d0 * DEXP(theta(i))/(1+DEXP(theta(i))) 
        i = i + 1
        sigp (3,3) = 1.0d0 * DEXP(theta(i))/(1+DEXP(theta(i))) 
        i = i + 1
		sigp (1,2) = DEXP(theta(i))/(1+DEXP(theta(i))) * SQRT(sigp(1,1)*sigp(2,2)) !covar wheat,sugar storage shock
		sigp (2,1) = sigp (1,2)
        i = i + 1
		sigp (1,3) = DEXP(theta(i))/(1+DEXP(theta(i))) * SQRT(sigp(1,1)*sigp(3,3)) !covar wheat,kero storage shock
		sigp (3,1) = sigp (1,3)
...
!!!!!!!!!!
    SUBROUTINE get_param (indi,mui,alphai,ximean,faultyweight,pf,logpm,tf,tm,tb,te)
    USE globvar
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: indi, mui
    REAL(8), INTENT(OUT) :: alphai(ncomm),ximean(ncomm),faultyweight,pf(ncomm),logpm(ncomm),tf,tm,tb,te
    REAL(8) :: alpha(ncomm)
    INTEGER :: i
